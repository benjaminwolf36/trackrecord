...
- Don't take our word for it: `grep -rE "fetch\(|node:https?|undici" node_modules/@trackrecord`
...
| `npx trackrecord card` | Renders a shareable 1200×630 ship-card PNG (`--since` / `--range` for any period) |
...
**Is this a "Wrapped"?**
The card works any day of the year over any range — it's a ship card, not a
year-in-review. Run it after a big week.
...
MIT. Fonts: JetBrains Mono, bundled under the [SIL OFL](packages/cli/assets/OFL.txt)
(bundling is part of the zero-network invariant).
